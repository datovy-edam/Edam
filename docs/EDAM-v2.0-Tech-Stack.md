...
> **Runtime note:** **BL-3.1 (Area P) COMPLETE — verified 2026-09-13.** The codebase now targets **.NET 10 (current LTS)**: plain libs `net10.0`, WinUI `net10.0-windows10.0.26100.0`, with Windows App SDK 1.6. Headless test suite re-verified green (26 pass / 2 skip). Remaining dependency-hygiene skew (EF Core 6.0.25, WinAppSDK-era packages) tracked under BL-5.5. **EF Core alignment (2026-09-18, BL-5.5):** EF Core **6.0.25 → 9.0.2** across every EF-referencing project (`Edam.Data.Lexicon`, `Edam.Data.Dictionary`, `Edam.Test.Dictionary`, `Edam.Studio`, `Edam.WinUI.Controls`) — builds 0 error and the EF-using packages **compile** against 9.0.2; EF 10's SqlServer provider is not cached (needs network). **Delivery caveat:** the Lexicon/Dictionary packages must be **republished with a SemVer bump** (they still pack as 1.0.0) before the app consumes EF-9-compiled assemblies. WinAppSDK-era skew remains. **Dependency-security cleanup (2026-09-18):** the `Edam.Studio.sln` NuGet Audit advisories were cleared with direct version pins — **5 of 6 fixed** (SqlClient 2.1.4→5.1.6, Caching.Memory 6.0.1→10.0.0, System.Drawing.Common 4.7.0→6.0.0, IdentityModel.Jwt/JsonWebTokens 6.8.0→8.19.2); the remaining `SQLitePCLRaw.lib.e_sqlite3` 2.0.4 (high) needs network access to obtain its fixed version. See **BL-5.7**.
