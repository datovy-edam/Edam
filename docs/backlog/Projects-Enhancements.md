...
> **Status:** Started (2026-09-18) — **PE-0…PE-3 complete**; PE-4…PE-5 planned.
...
**Evidence:** `Edam.Data.Projects.Conformance` → `result: PE-3 conformance ALL CONFORM` — `file-system` (14 checks), `catalog (local)` (13) and `catalog (remote HTTP)` (13), incl. *no process current-directory change*. (PE-2's single-provider run was 15 checks before the scenario was shared across back-ends.)
