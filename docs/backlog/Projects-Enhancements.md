...
> **Status:** Started (2026-09-18) — **PE-0…PE-4 complete**; PE-5 planned.
...
| **PE-5** | Consumers **and `IProjectRunner`**: bind the asset pipeline (`ProjectConsole`/`AssetServiceHelper`, plus the path readers `UriResourceInfo`/`FolderFileReader`/`ExcelDocumentReader`) to `IProjectResources`, then the Studio UI (`ProjectViewerViewModel`/`ProjectContext`) — **then retire the static project surface** (`Project`, `ProjectContext`, `ProjectConsole`, `ProjectHelper`, `ProjectDataModel`, `ApplicationHelper.ProjectInfo`) once no consumer references it | Pipeline reads/writes project resources through the seam (no CWD); the spec's example `*.Args.json` actually **runs** through `IProjectRunner`; UI browses/edits from the catalog; no static project state remains |

**Evidence:** `Edam.Data.Projects.Conformance` → `result: projects conformance (PE-2/PE-3/PE-4) ALL CONFORM` — **8 targets**: `file-system` (14 checks), `catalog (local)`, `catalog (remote HTTP)`, `catalog (postgres, local)`, `catalog (postgres, remote HTTP)`, **`di (file-system)`**, **`di (catalog, local file-system)`**, **`di (catalog, postgres)`** (13 each), incl. *no process current-directory change*. Pass a PostgreSQL DSN as the first argument to include the postgres targets (the default run is hermetic).
