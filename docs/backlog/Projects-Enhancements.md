...
| **PE-5** | Consumers + execution — in verified sub-steps: **5a** ✅ runner seam · **5b** ✅ asset-console adapter · **5c** real end-to-end process (**part 1 ✅**) + Studio UI on the DI-resolved providers (**part 2 remaining**) · **5d** retire the statics | **5a/5b done; 5c part 1 done 2026-09-18.** **5a:** runner reads the arguments through the seam, materializes inputs, runs, **captures outputs back** — verified on all targets, CWD unchanged. **5b:** the adapter runs `AssetServiceHelper` with the CWD contained and restored; binding `AddAssetConsoleProjectProcess()`. **5c part 1:** attempting a **real** process (XSD → `XsdToFile`) found + fixed two real defects — the adapter looked for the arguments file at the working-folder root instead of its **project-relative** path, and it called only `PrepareProceduresRegistry()` where the console needs its full **`Initialize()`** (session, appsettings, **type registry**, procedures). The real run now reports **`Asset Data Items expected but not found`** — the console needs real project data (its own test requires `ApplicationHelpers.InitializeApplication()` + `Projects/Datovy.HC.*`), so a self-contained fixture is insufficient; reported as **informational probes**, not pass/fail. **5d:** no static project state remains |
