...
**Evidence:** `Edam.Data.Projects.Conformance` → `result: PE-3 conformance ALL CONFORM` — `file-system` (14 checks), `catalog (local)`, `catalog (remote HTTP)`, `catalog (postgres, local)` and `catalog (postgres, remote HTTP)` (13 each), incl. *no process current-directory change*. Pass a PostgreSQL DSN as the first argument to include the postgres targets (the default run is hermetic).
