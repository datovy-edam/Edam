...
| **PE-5** | Consumers + execution — in verified sub-steps: **5a** ✅ runner seam · **5b** ✅ asset-console adapter · **5c** Studio UI (`ProjectViewerViewModel`/`ProjectContext` on the DI-resolved providers) · **5d** retire the statics | **5a/5b done 2026-09-18.** **5a:** the runner reads the arguments through the seam, materializes inputs, runs, and **captures outputs back into the project** — verified on all targets, CWD unchanged. **5b:** `AssetConsoleProjectProcess` loads the materialized arguments, runs `AssetServiceHelper` with the **current directory contained** to the working folder and **restored in `finally`**, and maps its results; binding is `AddAssetConsoleProjectProcess()`. Verified by substituting the console call (**6 checks**), and a **real invocation probe** reports `success=True` — the adapter genuinely reaches the pipeline — though that fixture produced **no output document**, so a properly configured process is needed for end-to-end artifact production (**5c**). **5d:** no static project state remains |

**Evidence:** `Edam.Data.Projects.Conformance` → `result: projects conformance (PE-2/PE-3/PE-4/PE-5a/PE-5b) ALL CONFORM` — **9 target groups** (`file-system` with 17 checks, the other eight with 16 each) plus **`asset-console adapter` (6 checks)**, incl. *no process current-directory change*. Pass a PostgreSQL DSN as the first argument to include the postgres targets (the default run is hermetic).
