...
| [BL-7.2](BL-7.2.md) | EF-independence + **PostgreSQL (Npgsql)** back-end behind `ICatalogStore`/`IContentStore`; **MS-SQL/EF deferred** | Refactor / architecture | **High** | L | **In Progress** (Npgsql provider + conformance verified — incl. **live-PostgreSQL ALL CONFORM 12/12**; Contracts-native REST client (3b-2); **`CatalogDb` fully retired** — ServiceClient (step 4), Testing + WinUI desktop local now Npgsql store-backed via `StoreBackedCatalogService` (step 5), service host; remaining: FileSystem `ICatalogStore` fixture) | **Ready** (Wave 1.1) |
