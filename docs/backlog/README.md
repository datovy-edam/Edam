...
| LM-2 | Container-scoped artifact addressing `(container, path)`: **2a** item index ✅ · **2b** content stores + Postgres composite key ✅ · **2b-ii** providers + wire share one scoping policy ✅ · **2c** `/<collectionId>/` prefix removed ✅ | Provider / migration | Medium | L | **COMPLETE 2026-09-24** — `scoping` **ALL CONFORM (11 checks)**; *"Project path is `/Projects/<name>` in every provider"* passes in all 8 provider/DI targets; full matrix incl. Postgres green |
