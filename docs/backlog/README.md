...
| [BL-7.4](BL-7.4.md) | DI seam + per-**Container** provider resolution (`ContainerBinding`/`ICatalogProviderResolver`); drop factory `switch` | Refactor / infrastructure | Medium | M | **In Progress** (composition root + per-Container resolver delivered + verified — conformance-through-DI ALL CONFORM; service host DI-resolved; **WinUI→DI consumer migration pending**; PostgreSql `EnlistContainer` id fix) | **Ready** (Wave 1.1) |
