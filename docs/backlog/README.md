...
| LM-2 | Container-scoped artifact addressing `(container, path)`: **2a** item index ✅ · **2b** content stores + Postgres composite key ✅ · **2b-ii** providers + wire share one scoping policy ✅ · **2c** drop the `/<collectionId>/` path prefix ⏳ | Provider / migration | Medium | L | **2a + 2b + 2b-ii Done 2026-09-24** — `scoping (LM-2a/2b)` group **ALL CONFORM (11 checks)** incl. the projects-layer isolation proof; catalog + HTTP conformance re-run green |
