...
| LM-2 | Container-scoped artifact addressing `(container, path)`: **2a** item index ✅ · **2b** content namespace + Postgres composite key ✅ · **2b-ii** hand per-container stores to the providers ⏳ · **2c** drop the `/<collectionId>/` path prefix ⏳ | Provider / migration | Medium | L | **2a+2b Done 2026-09-24** — `scoping (LM-2a/2b)` group **ALL CONFORM (10 checks)**: two containers hold the same item path *and* the same content path, each its own; Postgres migrated to the `(container_id, resource_path)` key (verified read-only against the live DB) |
...
| LM-6 | **Consumer migration** — first consumers: the Studio bridge reads through `ProjectSettings`; its new-project flow **seeds** again; a `host` group proves the consumer story from configuration. Remaining: the static pipeline surface (needs a user-side validation) + the 12 settings copies | Refactor | Medium | L | **In Progress 2026-09-24** — `host (LM-6)` group **ALL CONFORM (6 checks)**; `Edam.Studio.sln` 0 error |
