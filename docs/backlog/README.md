...
| LM-2 | Container-scoped artifact addressing `(container, path)`: **2a** item index scoped ✅ · **2b** content namespace + Postgres composite key ⏳ · **2c** drop the `/<collectionId>/` path prefix ⏳ | Provider / migration | Medium | L | **2a Done 2026-09-24** — `scoping (LM-2a)` group **ALL CONFORM (5 checks)**: two containers may hold the same path, each with its own item (the PE-3 defect fixed). **2b needs the database up** |
