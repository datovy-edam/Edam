...
## Progress
- (2026-09-18) **Model folder client retired (deleted) — folder coverage moved to the indexer.** Deleted `FolderCatalogClient` + `FolderCatalogItem`/`FolderCatalogItemData` and the orphaned Model `CatalogFileSystem` helper, plus the two `Edam.Test.TestBuilder` harnesses that only exercised them (they encoded the retired client's semantics and could not be migrated faithfully). Replaced their coverage with `Edam.Data.Catalog.Tests/FolderCatalogIndexerTests` (items + content + idempotent re-index + missing-folder no-op). `CatalogClone` (Model) is kept as an orphaned capability — unused is not the same as dead. All builds 0 error; HTTP conformance still ALL CONFORM (filesystem + postgres).
- (2026-09-18) **Folder-as-catalog ported behind the Contracts seams (folder ingestion) — verified.** New `Edam.Data.Catalog.Folder` (Contracts-only) with `FolderCatalogIndexer`: a folder is **ingested into a catalog** (directories → branches, files → leaves, bytes → `IContentStore` at the same path) by driving the Contracts seams directly, so the same call works against PostgreSQL, FileSystem, or a remote `ICatalogClient`. Deterministic ids make re-indexing idempotent. The WinUI `FileSystem`-container path resolves store+content via DI (`CatalogServiceHelper.GetLocalStore`) and indexes the folder instead of `new FolderCatalogClient(...)`. **Verified:** three new HTTP-conformance checks (index a real folder; items visible over the API; content visible over the API) → **ALL CONFORM (23 checks) on filesystem + postgres**; all consumer builds 0 error.
