...
> **Status:** Started — **LM-1 ✅** (address core, 19) · **LM-3 ✅ core** (settings reader, 12) · **LM-4 ✅** (bindings, 9) · **LM-5 ✅** (seeding, 6) · **LM-6 ⏳** (Studio bridge + seeding wired; `host` 6) · **LM-2 ✅ COMPLETE 2026-09-24** (a item index · b content stores + Postgres composite key · b-ii one scoping policy for providers/wire · c the `/<collectionId>/` prefix removed — every provider now addresses a project as `/Projects/<name>`) · **LM-0 done (a)+(b)** · **next: the rest of LM-6** (the static pipeline surface needs a user-side validation; the 12 settings copies) → **LM-7**.
...
| **LM-2** | **Container-scoped artifact addressing `(container, path)`** — ✅ **COMPLETE 2026-09-24**: **LM-2a** (item index keyed by container + path, matched inside its container) · **LM-2b** (container-scoped content stores + the Postgres `(container_id, resource_path)` key via an additive migration) · **LM-2b-ii** (`CatalogScopedContent` — one scoping policy shared by the providers, the DI root and the **wire**; the sentinel `default` = the legacy unscoped namespace) · **LM-2c** (the projects-side `/<collectionId>/` prefix is **gone**: a project is `/Projects/<name>` in **every** provider, because the container is part of the **address**, not the path — the prefix had become load-bearing, so removing it exposed and fixed a container-blind lookup in the providers) | two containers may hold identical paths — for **any** artifact — and single-container content still resolves; providers now agree on one address | **`scoping (LM-2a/2b)` group — ALL CONFORM (11 checks)** + *"Project path is /Projects/&lt;name&gt; in every provider"* passing in all 8 provider/DI targets; full matrix incl. Postgres ALL CONFORM | medium-high |
