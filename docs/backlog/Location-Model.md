...
> **Status:** Started — **LM-1 DONE** (address core, 19) · **LM-3 core DONE** (settings reader, 12) · **LM-4 DONE** (bindings, 9) · **LM-5 DONE** (seeding, 6) · **LM-6 IN PROGRESS** (Studio bridge + seeding wired; `host` 6) · **LM-2a DONE 2026-09-24** (container-scoped item index, 5) · **LM-0 done (a)+(b)** · **next: LM-2b** (content scoping + the Postgres composite key — needs the DB up), **LM-2c** (drop the path prefix), the rest of **LM-6**, **LM-7**.
...
| **LM-2** | **Container-scoped artifact addressing `(container, path)`** — **LM-2a ✅ DONE 2026-09-24**: the catalog's **item index is keyed by container + path** and an existing branch is matched **inside its container** (`FileSystemCatalogStore`, the in-memory conformance store, and the Postgres lookup). **LM-2b ⏳**: the **content** side (`IContentStore` is path-only, so two containers share one blob namespace) and the PostgreSQL global `resource_path` **primary key** → composite key + migration — **needs the 2a/2b instance-or-contract decision and the database up** (Postgres was unreachable when 2a was done). **LM-2c ⏳**: drop the projects-side `/<collectionId>/` path prefix once content is scoped | two containers may hold identical paths — for **any** artifact — and single-container content still resolves | **`scoping (LM-2a)` group — ALL CONFORM (5 checks)**; the catalog conformance and the catalog HTTP conformance unchanged | medium-high |
