...
> **Status:** Started — **LM-1 DONE** (address core, 19 checks) · **LM-3 core DONE** (settings reader + legacy translation, 12 checks) · **LM-4 DONE** (bindings + environment overrides, 9 checks incl. the payoff test) · **LM-5 DONE** (seeding, 6 checks) · **LM-6 IN PROGRESS 2026-09-24** (first consumers migrated: the Studio bridge reads through `ProjectSettings` and its new-project flow seeds again; a `host` group proves the consumer story from configuration — 6 checks) · **LM-0 done (a)+(b)** · next: **LM-2** (needs the 2a/2b call), the rest of **LM-6** (pipeline statics — needs a user-side validation; the 12 settings copies), **LM-7**.
