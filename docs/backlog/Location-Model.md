...
> **Status:** Started — **LM-1 DONE (2026-09-24; `address` conformance group ALL CONFORM, 19 checks)**; **LM-0 partial** (seed source sanitized; seed *wiring* + settings dedupe still pending because those files are mid-change in the working tree); **next: LM-3/LM-4**, with **LM-2** needing the 2a/2b decision.
...
|---|---|---|---|---|
| **LM-0** *(safe, do first)* | **Settings debt cleanup — data only, no schema change** *(step 1 done 2026-09-24; steps 2–3 pending)*: (a) ✅ sanitize the **seed source** `app-data/Edam.Settings.json` — removed the two machine-specific absolute paths (`C:\prjs\Datovy.Edam/…`, `C:\Users\esobr\Documents/…`), the third collection entry, the committed connection string, and the stale mixed-separator `ConsolePath`; (b) ⏳ **re-wire the packaged seed** — `Edam.Studio.csproj` includes `ApplicationData\**` but that folder no longer exists and nothing includes `app-data/`, so a clean build packages **no seed** (pending: that csproj is mid-change in the working tree); (c) ⏳ collapse the 13 duplicated location keys in `appsettings.json` | no absolute machine path and no secret in any committed settings file; a **clean** build packages the seed | grep + a settings-vs-folders check + a clean-build check of the packaged seed | **none** (data/build wiring only) — and it must not be mistaken for the fix |
