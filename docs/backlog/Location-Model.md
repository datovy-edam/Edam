...
> **Status:** Started — **LM-1 ✅** (address core, 19) · **LM-3 ✅ core** (settings reader, 12) · **LM-4 ✅** (bindings, 9) · **LM-5 ✅** (seeding, 6) · **LM-6 ⏳** (Studio bridge + seeding wired; `host` 6) · **LM-2a ✅ · LM-2b ✅ · LM-2b-ii ✅ 2026-09-24** (container-scoped items *and* content, one policy shared by the providers and the service; `scoping` 11 checks against the live database) · **LM-0 done (a)+(b)** · **next: LM-2c** (drop the path prefix) → the rest of **LM-6** → **LM-7**.
...
| **LM-2** | **Container-scoped artifact addressing `(container, path)`** — **LM-2a ✅ (item index)**, **LM-2b ✅ (content stores + Postgres composite key + additive migration)**, **LM-2b-ii ✅ DONE 2026-09-24 (the providers and the service share ONE scoping policy)**: `CatalogScopedContent.Factory(config)` builds a **container-scoped** content store from the resolved storage (the sentinel **`default` = the legacy unscoped namespace**), `IProjectContentStoreResolver` hands it to `CatalogProjectResources`/`CatalogProjectStore` **per operation**, and the **wire** carries the container (`/catalogservice/content/*?containerId=…` + `CatalogHttpContent` sending the client's container) so **local and remote access share one namespace**. ⏳ **LM-2c**: drop the projects-side `/<collectionId>/` path prefix (no longer *needed*; it changes existing project paths, so it is its own step) | two containers may hold identical paths — for **any** artifact — and single-container content still resolves | **`scoping (LM-2a/2b)` group — ALL CONFORM (11 checks)**, incl. the live-Postgres payoff and the **projects-layer** isolation check (same project path in two containers, no prefix involved); the full postgres matrix and the catalog suites unchanged | medium-high |
