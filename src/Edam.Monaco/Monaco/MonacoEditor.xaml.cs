...
public sealed partial class MonacoEditor : UserControl, IMonacoEditor, IMonacoCore
{
   ...
   /// <summary>
   /// Raised when the user requests a save (Ctrl-S / Cmd-S) from the editor.
   /// </summary>
   ...
   private async void MonacoEditorParentView_Loaded(object sender, RoutedEventArgs e)
   {
      ...
      await InitializeEditorModelAsync();
   }
   ...
   private async Task InitializeEditorAsync()
   {
      ...
      WebView_NavigationCompleted(this, null);
   }
   ...
   private void ProcessMonacoEvents(string monacoEvent)
   {
      switch (monacoEvent)
      {
         ...
         case "EVENT_EDITOR_SAVE_REQUESTED":
            {
               EditorSaveRequested?.Invoke(this, EventArgs.Empty);
            }
            break;
