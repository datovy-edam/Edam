...
namespace Edam.Diagnostics
{
   ...
   public class ResultLog : IResultsLog
   {
      ...
      public Verbosity Verbosity { get; set; }
       ...
       /// <summary>
       /// BL-4.1: Microsoft.Extensions.Logging (MEL) logger for this result log.
       /// Additive/non-breaking — null logger unless a concrete ILogger is injected.
       /// </summary>
       public Microsoft.Extensions.Logging.ILogger Logger { get; set; } =
          Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance;

       /// <summary>
       /// BL-4.1: bind this result log to an MEL logger by category at a composition root
       /// (DI). Non-breaking — no-op-safe; the result envelope is unchanged.
       /// </summary>
       public void UseLogging(Microsoft.Extensions.Logging.ILoggerFactory factory)
       {
          if (factory != null)
             Logger = factory.CreateLogger(GetType().FullName ?? GetType().Name);
       }

       /// <summary>
       /// BL-4.1: map EDAM severity to a MEL LogLevel.
       /// </summary>
       public static Microsoft.Extensions.Logging.LogLevel ToLogLevel(
          SeverityLevel severity)
       {
          switch (severity)
          {
             case SeverityLevel.Debug: return Microsoft.Extensions.Logging.LogLevel.Debug;
             case SeverityLevel.Warning: return Microsoft.Extensions.Logging.LogLevel.Warning;
             case SeverityLevel.Critical: return Microsoft.Extensions.Logging.LogLevel.Critical;
             case SeverityLevel.Fatal: return Microsoft.Extensions.Logging.LogLevel.Critical;
             default: return Microsoft.Extensions.Logging.LogLevel.Information;
          }
       }

       /// <summary>
       /// BL-4.1: forward a message-log entry to the MEL logger (additive; no-op on null logger).
       /// </summary>
       public void LogMEL(IMessageLogEntry entry)
       {
          if (entry == null || Logger == null)
             return;

          String msg = entry.Message ?? String.Empty;
          Microsoft.Extensions.Logging.LogLevel level = ToLogLevel(entry.Severity);
          Exception ex = entry.Exception as Exception;
           if (ex != null)
             Microsoft.Extensions.Logging.LoggerExtensions.LogError(
                Logger, ex, "{Edam}");
          else
             if (level == Microsoft.Extensions.Logging.LogLevel.Debug) { s_TraceMEL(Logger, msg, null); } else { Microsoft.Extensions.Logging.LoggerExtensions.Log(Logger, level, msg); }
       }

      /// <summary>
      /// BL-4.1: pre-compiled, high-performance structured trace template (LoggerMessage),
      /// used by <see cref="LogMEL"/>. The source-generated [LoggerMessage] form is NOT viable
      /// here: Edam.System ships its own <c>Edam.Diagnostics.Logger</c> type, and that name
      /// collision (BL-4.2) breaks the source generator's name resolution. The runtime
      /// <see cref="Microsoft.Extensions.Logging.LoggerMessage.Define{T1}(EventId, LogLevel, string)"/>
      /// equivalent caches the message template and avoids boxing/string allocation without the collision.
      /// </summary>
      private static readonly System.Action<
         Microsoft.Extensions.Logging.ILogger, string, System.Exception> s_TraceMEL =
      ...
      public void Add(IMessageLogEntry message)
      {
         ...
         // BL-4.1: forward to MEL logger (additive; no-op on null logger).
         LogMEL(message);
      }  // end of Add message
