...
namespace Edam.Diagnostics
{
   ...
   public class Log
   {
      ...
      // ---- BL-4.1: MEL composition-root hook for the legacy static Log facade ----
      // EVENT_LOG_SUPPORT is intentionally NOT compiled. Instead of silently dropping
      // messages, a MEL logger bound here (at a composition root) receives them.
      private static Microsoft.Extensions.Logging.ILogger m_MelLogger =
         Microsoft.Extensions.Logging.Abstractions.NullLogger.Instance;

      /// <summary>
      /// BL-4.1: bind the static <see cref="Log"/> facade to an MEL logger factory at a
      /// composition root so <see cref="Write(LogSettings, SeverityLevel, string, string)"/>
      /// forwards to MEL providers. Non-breaking: no factory bound => legacy behavior.
      /// </summary>
      public static void UseLogging(Microsoft.Extensions.Logging.ILoggerFactory factory)
      {
         if (factory != null)
            m_MelLogger = factory.CreateLogger(typeof(Log).FullName ?? typeof(Log).Name);
      }
      ...
      static public bool Write(
         ...
         String Message)
      {
         // BL-4.1: EVENT_LOG_SUPPORT is not compiled, so instead of a silent no-op we
         // forward the entry to the MEL logger bound at a composition root (UseLogging).
         // Falls back to false (legacy behavior) when no MEL logger is attached.
         if (Level == SeverityLevel.Unknown ||
             String.IsNullOrEmpty(Message) ||
             m_MelLogger == null)
            return false;

         String loc = String.IsNullOrEmpty(Location) ? String.Empty : "(" + Location + ") ";
         Microsoft.Extensions.Logging.LoggerExtensions.Log(
            m_MelLogger, ResultLog.ToLogLevel(Level), loc + Message);
         return true;
      }
